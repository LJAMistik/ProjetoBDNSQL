...
// Nova rota para filtros com operadores de comparação e lógico
router.get('/filter', async (req, res) => {
  const { minClassificacao, maxClassificacao, especialidade } = req.query

  const query = {
    $and: [
      { classificacao: { $gte: parseFloat(minClassificacao) || 0 } },
      { classificacao: { $lte: parseFloat(maxClassificacao) || 10 } },
  ...
  try {
    ...
    res.status(500).json({
      message: 'Erro ao obter a listagem das clínicas com os filtros',
      error: `${err.message}`
    })
  }
})
